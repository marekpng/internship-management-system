...
async function submit() {
  ...
  if (!form.street) {
    error.value = 'Ulica je povinná.'
    return
  }
  if (!form.house_number) {
    error.value = 'Číslo domu je povinné.'
    return
  }
  if (!form.city) {
    error.value = 'Mesto je povinné.'
    return
  }
  if (!form.postal_code) {
    error.value = 'PSČ je povinné.'
    return
  }
  ...
  try {
    ...
    success.value =
      res.data.message || 'Registrácia prebehla úspešne. Skontrolujte email (aktivačný link).'
